...
class VGGAutoEncoder(nn.Module):
    ...
    def __init__(self, layer_cfg):
        ...
        self.encoder = VGGEncoder(layer_cfg=layer_cfg,       enable_bn=True)
        self.decoder = VGGDecoder(layer_cfg=layer_cfg[::-1], enable_bn=True)
...
class VGG(nn.Module):
    ...
    def __init__(self, layer_cfg, num_classes, dropout, img_size, enable_bn=False):
        ...
        self.encoder = VGGEncoder(layer_cfg=layer_cfg, enable_bn=enable_bn)
...
class VGGEncoder(nn.Module):
    ...
    def __init__(self, layer_cfg, enable_bn=False):
...
if __name__ == "__main__":
    ...
    layer_cfg = get_configs()

    model = VGGAutoEncoder(layer_cfg)
